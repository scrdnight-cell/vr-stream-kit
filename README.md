...
| Ending a session badly crashes Bigscreen | Changing displays under a live stream, or killing Bigscreen in the middle of one, crashed it in testing. | **R** tears the stream down first, then closes Bigscreen, then hands the display back — and checks Windows for a GPU reset, by when it happened rather than when it was reported (see [Troubleshooting](#troubleshooting)). |
...
**The picture cuts out when I switch the monitor on or off.** That is the restart described in [Following the display](#following-the-display) — Bigscreen is rebuilding its capture around the displays that are attached now, which takes about 16 seconds on the test system. `logs\session.log` names each one and how long it took. If you would rather stay on the virtual display when a monitor comes back, set `ON_MONITOR_RETURN=virtual` in the launcher.

**The headset says the PC is disconnected or unpaired during a restart.** Leave it. For a few seconds there is no Bigscreen on the PC for the headset to be paired with; the new one comes up and the headset reconnects by itself. Starting a manual re-pair at that moment is the one thing that can get in the way of the reconnect.
...
**"GPU RESET" in Event Viewer right after a program crashes.** Windows Error Reporting re-files its queue of old GPU reports whenever any program crashes, and in a burst after a reboot. On the test machine, every one of 47 crashes over two days — Bigscreen and a mouse utility alike — was followed 1–5 seconds later by the same 12 reports, dated December 2024 to the day before, and not one of them was a reset from that day. It looks exactly like "a crash resets the GPU", and it is not. So a *LiveKernelEvent 141* entry's time is when it was *reported*, not when the GPU reset. Each report names its dump file, and the name carries the real moment: `WATCHDOG-20260916-0948.dmp`. The kit's shutdown check uses that, not the report time.
